...
function WorkflowPresetListView() {
          ...
          if (group.id === itemId) {
            ...
            group.workflowChild?.forEach(child => {
              if (child.id === itemId) {
                ...
                const hasOtherSelectedChildren = group.workflowChild?.some(sibling => 
              ...
              child.workflowChild?.forEach(grandChild => {
                if (grandChild.id === itemId) {
                  ...
                  const hasOtherSelectedGrandChildren = child.workflowChild?.some(sibling =>
                    sibling.id !== itemId && newSet.has(sibling.id)
                  );
  ...
  useEffect(() => {
    if (isCreateOpen && CategoryId && tokenData && DataWorkflowGroups.length === 0) {
                        ...
                        <VStack align="start" spacing={3}>
                          ...
                          <VStack align="start" spacing={1} w="full">
                            ...
                            <Text fontSize="sm" color="gray.500" noOfLines={2}>
                              {preset.wfPresetDesc || "No description"}
                            </Text>
                            <Text fontSize="xs" color="gray.400">
                              Category: {preset.wfCategoryName}
                            </Text>
                          </VStack>
                          ...
                          <HStack justify="space-between" w="full" pt={2}>
                            ...
                            <Badge colorScheme="green" size="sm">
                              {preset.workflowData?.length || 0} workflows
                            </Badge>
                          </HStack>
                        </VStack>
                      </CardBody>
                ...
                <GridItem>
                  <Box>
                    <Text mb={3} fontSize="sm" fontWeight="medium">
                      Select Workflows * ({DataWorkflowGroups.length} available)
                    </Text>
                    {DataWorkflowGroups.length === 0 ? (
                      <Box
                        p={4}
                          ...
                          <Box key={group.id} w="full">
                            ...
                            >
                              <HStack spacing={2} align="center">
                                <Checkbox
                                  isChecked={selectedWorkflows.has(group.id)}
                                  onChange={(e) =>
