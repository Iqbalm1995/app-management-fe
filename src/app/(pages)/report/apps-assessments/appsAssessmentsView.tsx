...
export default function AppsAssessmentsView() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const fw: any[] = [];
      if (filterQ)
        fw.push({ field: "quartalReport", operator: "=", value: filterQ });
      if (filterYear)
        fw.push({ field: "yearReport", operator: "=", value: filterYear });
      // Tab-locked status filter
      fw.push({
        field: "statusReport",
        operator: "in",
        value: TAB_STATUS[tabMode],
      });
  ...
  const handleGenerate = async () => {
    ...
    if (res?.statusCode === RES_CODE_OK) {
      showToast({
        description: `Generated ${res.data?.batchCode} — ${res.data?.assessmentCount} assessments`,
        statusToast: "success",
      });
      setRefresh((p) => p + 1);
    } else
      showToast({
        description: res?.message || "Generate failed",
        statusToast: "error",
      });
  };
      ...
      },
      {
        ...
        cell: (info) => (
          <Button
            ...
            onClick={() =>
              router.push(
                `/report/apps-assessments/detail?batchCode=${info.row.original.batchCode}`,
              )
            }
          >
            Show
  ...
  return (
    <LayoutAdmin>
      <HeaderContent
        titleName="Assessment of Critical Apps"
        breadCrumb={["Home", "Report", "Assessment of Critical Apps"]}
      />
      <Box p={4}>
        <Card
          rounded={radiusStyle}
                ...
                <HStack spacing={3}>
                  ...
                  <VStack align="start" spacing={0}>
                    <Heading size="md" color={isDark ? "white" : "gray.800"}>
                      Report Assessment Apps
                    </Heading>
                    <Text
                      fontSize="sm"
                      color={isDark ? "gray.400" : "gray.600"}
                    >
                      {totalCount} batch(es) found
                    </Text>
                  </VStack>
                </HStack>
                <Button
                  display={!canGenerate ? "none" : "block"}
                  colorScheme="purple"
                  leftIcon={generating ? <Spinner size="xs" /> : <FiZap />}
                  ...
                  title={
                    !canGenerate
                      ? "Your group does not have permission to generate reports"
                      : undefined
                  }
                >
                  Generate Report
              ...
              >
                <Button
                  ...
                  onClick={() => {
                    ...
                    router.replace("/report/apps-assessments?tab=ONGOING");
                  }}
                >
                  On Going Review
                </Button>
                <Button
                  ...
                  px={4}
                  onClick={() => {
                    setTabMode("COMPLETED");
                    setPagination({ pageIndex: 0, pageSize });
                    router.replace("/report/apps-assessments?tab=COMPLETED");
                  }}
                >
                  Completed
                </Button>
            ...
            <VStack spacing={4} align="stretch">
              ...
              >
                ...
                <VStack align="start" spacing={0}>
                  ...
                  >
                    This will generate a new assessment batch for the current
                    quarter and year.
                  </Text>
                </VStack>
              </HStack>
              ...
              <SimpleGrid columns={2} spacing={3}>
                {[
                  ...
                  {
                    label: "Batch Code",
                    value: `RPQ${currentQuarter}${currentYear}-APPS????`,
                  },
                  { label: "Status", value: "DRAFT" },
                  {
                    label: "Applications",
                    value:
                      appCount === null ? "Loading..." : `${appCount} apps`,
                  },
                ].map(({ label, value }) => (
                  <Box
                    key={label}
                  ...
                  >
                    ...
                    <Text
                      fontSize="sm"
                      fontWeight="semibold"
                      fontFamily={label === "Batch Code" ? "mono" : "inherit"}
                    >
                      {value}
                    </Text>
                  </Box>
              ...
              <Text fontSize="xs" color={isDark ? "gray.400" : "gray.500"}>
                A new batch will be created for ALL active applications. Each
                app will get an assessment record with all criteria to be filled
                in.
              </Text>
            </VStack>
          </ModalBody>
