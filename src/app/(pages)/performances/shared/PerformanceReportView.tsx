...
export type PerformanceReportMode = "division" | "group" | "team";
...
export default function PerformanceReportView({ mode }: PerformanceReportViewProps) {
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      // Fetch ALL data with current filters (ignore pagination)
      ...
      if (blob) {
        ...
        const filename = `User_Evaluation_Report_${selectedYear}_Q${selectedQuarters.join("-")}_${currentDate}.xlsx`;
  ...
  useEffect(() => {
    ...
    // Only add year filter if not "All" (0)
    if (selectedYear > 0) {
    ...
    // Only add quarter filter if year is set and single quarter selected
                ...
                <GridItem colSpan={{ base: 12, lg: 3 }}>
                  <Flex alignItems={"center"} gap={3}>
                    ...
                    <Select
                      ...
                      isDisabled={mode === "group" || mode === "team"}
                      value={
                        groupOptions.find(
                ...
                <GridItem colSpan={{ base: 12, lg: 3 }}>
                  <Flex alignItems={"center"} gap={3}>
                    ...
                    <Select
                      ...
                      isDisabled={mode === "team" || !FilterManageGroup}
                      isClearable
                    />
                  </Flex>
                </GridItem>
