...
export default function DashboardPortfolioRealTimePage() {
  ...
  const [characteristicsData, setCharacteristicsData] = useState<
    ProjectCharacteristicsDashboardResponse[]
  >([]);
  const [characteristicsTotalProjects, setCharacteristicsTotalProjects] =
    useState<number>(0);
  ...
  const [projectAcquisitionsData, setProjectAcquisitionsData] = useState<
    ProjectAcquisitionsDashboardResponse[]
  >([]);
  const [acquisitionsTotalProjects, setAcquisitionsTotalProjects] =
    useState<number>(0);
  ...
  const {
    ...
    requirementType,
    requirementSummary,
    requirementDivisionSender,
    requirementMemoSummary,
    getRequirementTypeDashboard,
  ...
  const getMemoChartData = (memoFlag: "Y" | "N") => {
    ...
    const brdData = quarterMonths.map(({ monthPeriod }) => {
      const item = filteredData.find(
        (d: any) =>
          d.monthPeriod === monthPeriod && d.requirementType === "BRD",
      );
    ...
    const rfcData = quarterMonths.map(({ monthPeriod }) => {
      const item = filteredData.find(
        (d: any) =>
          d.monthPeriod === monthPeriod && d.requirementType === "RFC",
      );
      return item?.requirementCount || 0;
    });
  ...
  const loadDivisionData = async () => {
    ...
    try {
      ...
      const latestMonthPeriod = Math.max(
        ...apiData.map((item) => item.monthPeriod),
      );
  ...
  const loadCharacteristicsData = async () => {
    ...
    try {
      ...
      const actualCharTotal = apiData.reduce(
  ...
  const loadProjectAcquisitionsData = async () => {
    ...
    try {
      ...
      const actualAcqTotal = apiData.reduce(
        (sum, item) => sum + item.projectCount,
        0,
      );
  ...
  const loadDevStaffProjectClosedData = async () => {
    ...
    try {
      ...
      const userGroups = apiData.reduce(
        (acc, item) => {
  ...
  const loadRequirementTypeData = async () => {
    ...
    try {
      ...
      console.error("Failed to load requirement type data:", err);
    }
  };
  ...
  const loadRequirementSummaryData = async () => {
    ...
    try {
      ...
      console.error("Failed to load requirement summary data:", err);
    }
  };
  ...
  const loadRequirementDivisionData = async () => {
    ...
    try {
      ...
      console.error("Failed to load requirement division data:", err);
    }
  };
  ...
  const loadRequirementMemoData = async () => {
    ...
    try {
      ...
      console.error("Failed to load requirement memo data:", err);
    }
  };
  ...
  return (
    <LayoutAdmin>
      ...
      <Box p={6}>
                      ...
                      {projectAcquisitionsData.length > 0 ? (
                        <Box>
                          <Chart
                            ...
                            height={Math.max(
                              400,
                              projectAcquisitionsData.length * 65,
                            )}
                          />
                          <HStack justify="center" mt={4} spacing={6}>
                            <Stat textAlign="center" size="sm">
                      ...
                      {projectByGroupManageData.length > 0 ? (
                        <Box>
                          <Chart
                            ...
                            height={Math.max(
                              400,
                              projectByGroupManageData.length * 65,
                            )}
                          />
                          <HStack justify="center" mt={4} spacing={6}>
                            <Stat textAlign="center" size="sm">
                      ...
                      options={{
                        ...
                        xaxis: {
                          ...
                          categories: acquisitionsModalData.map((item) => {
                            const name =
                              item.projectAcquisitionName || "Unknown";
                            const words = name.split(" ");
                            const lines: string[] = [];
                            let current = "";
                            words.forEach((w) => {
                      ...
                      options={{
                        ...
                        xaxis: {
                          ...
                          categories: groupManageModalData.map((item) => {
                            const name =
                              item.projectGroupNameManage || "Unknown";
                            const words = name.split(" ");
                            const lines: string[] = [];
                            let current = "";
                            words.forEach((w) => {
        ...
        >
          <ModalOverlay />
          <ModalContent bg={colorMode === "light" ? "gray.50" : "gray.800"}>
