...
import { PROJECT_CLOSE_STATUS_OPTIONS } from "@/app/constants/masterStatusConstants";
import {
...
function ProjectClosePortfolioReportPage() {
  ...
  const { ListProjectClosePortofolio, isLoading: reportLoading } = useReports();
  ...
  const handleExport = async (exportType: "XLSX" | "PDF") => {
    ...
    await requestExport({
      moduleName: "PROJECT_CLOSE_PORTFOLIO",
      exportType: exportType,
      reportTitle: `Project Close Portfolio Report (${FilterQuarter ? `Q${FilterQuarter}` : "All Q"} ${FilterYear || ""})`,
      ...
      <Card borderRadius={radiusStyle} mb={4}>
        <CardHeader>
          <Flex justify="space-between" align="center">
            <Heading size="md">Project Close Portfolio</Heading>
          ...
          <Grid templateColumns="repeat(5, 1fr)" gap={4} mb={4}>
            ...
            <GridItem>
              ...
              <Select
                ...
                rounded={radiusStyle}
              >
                {PROJECT_CLOSE_STATUS_OPTIONS.map((status) => (
