            ...
            <CardBody>
              <Box minH={"80vh"}>
                <Flex as={Stack} w={"full"} p={2} divider={<StackDivider />}>
                  ...
                  <Flex w={"full"} minH={"40vh"} p={2} id={"teamRoleSettings"}>
                    <TeamRoleProps />
                  </Flex>
...
const TeamProfileSettingProps = () => {
  ...
  const [image, setImage] = useState("/img/placeholder-header-sm.png"); // Default image
...
const TeamMembersProps = () => {
  ...
  const [TeamMembers, setTeamMembers] = useState<TeamsUserMemberResponse[]>([]);
  const [RefreshData, setRefreshData] = useState<number>(0);
  ...
  const columnsData = useMemo<ColumnDef<TeamsUserMemberResponse>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: (info) => (
          <Stack spacing={0}>
            <Text>
              {info.row.original.userFirstName} {info.row.original.userLastName}
            </Text>
            <Text
              fontWeight={600}
              fontSize={"xx-small"}
            ...
            <Text
              ...
              color={"secondary.300"}
            >
              ({info.row.original.role.roleCode})
              {info.row.original.role.roleName}
            </Text>
          </Stack>
        ),
        ...
        cell: (info) => (
          <Flex w={"full"} as={Wrap} justifyContent={"center"}>
            <Button
              ...
              isLoading={ActionLoading || DataOptionsRoleTeam.length <= 0}
            >
              <FiEdit3 />
            </Button>
            <Button
    ...
    if (DataAuth && DataAuth.teamMember) {
      ...
      const GetDataList = async () => {
        ...
        if (isErrorResponse || !requestData) {
          ...
          const itemsData: TeamsUserMemberResponse[] =
            requestData.data as TeamsUserMemberResponse[];
  ...
  useEffect(() => {
    ...
    if (DataAuth && DataAuth.teamMember) {
      ...
      setIsLoadingProcess(true);
      const GetDataList = async () => {
        const requestData = await ListRoleTeam(PayloadList, tokenData);
        const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
        ...
        if (isErrorResponse || !requestData) {
          ...
          const itemsData: TeamRoleFullResponse[] =
            requestData.data as TeamRoleFullResponse[];
          ...
          if (itemsData.length > 0) {
            itemsData.map((dt, index) => {
              setDataOptionsRoleTeam((prevData) => [
                ...prevData,
                {
                  label: `${dt.teamRoleName} (${dt.teamRoleCode})`,
                  value: dt.id,
                },
              ]);
            });
  ...
  const DeleteTeamMemberServ = async (data: TeamMemberPayload) => {
    const requestData = await DeleteTeamMember(data, tokenData);
    const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
  ...
  const handleEditData = async (data: TeamsUserMemberResponse) => {
    setActionLoading(true);
    console.log(data);
    if (data.teamRole != null && DataOptionsRoleTeam.length > 0) {
      ...
      formik.setFieldValue("teamId", data.team.id);
  ...
  const [questionMsgDialog, setQuestionMsgDialog] = useState<string>("");
  const [captionDialog, setCaptionDialog] = useState<string>("");
  const [detailData, setdetailData] = useState<TeamsUserMemberResponse | null>(
    null
  );
  ...
  const handleConfirmDeleteData = (data: TeamsUserMemberResponse) => {
    setCaptionDialog("Confirm Delete");
    setQuestionMsgDialog(
      `Are you sure want to delete "${data.userFirstName}"?`
    );
  ...
  const handleDeleteData = async () => {
    setActionLoading(true);
    await delay(DELAY_MEDIUM);
    if (DataAuth && DataAuth.teamMember && detailData && detailData.teamRole) {
      await DeleteTeamMemberServ({
        teamId: detailData.team.id,
        teamRoleId: detailData.teamRole.id,
...
const TeamRoleProps = () => {
  ...
  const columnsData = useMemo<ColumnDef<TeamRoleFullResponse>[]>(
    () => [
      {
        accessorKey: "numbData",
        cell: (info) => (
      ...
      {
        accessorFn: (row) => row.teamRoleCode,
        id: "teamRoleCode",
        cell: (info) => (
          <Stack spacing={0}>
            <Text>{info.row.original.teamRoleName}</Text>
            <Text
              fontWeight={600}
              fontSize={"xx-small"}
              color={"secondary.700"}
            >
              {info.row.original.teamRoleCode}
            </Text>
          </Stack>
        ),
        header: () => <span>User Member</span>,
      ...
      {
        accessorFn: (row) => row.teamCode,
        id: "teamCode",
        cell: (info) => (
          <Stack spacing={0}>
            <Text>{info.row.original.teamName}</Text>
            <Text
              fontWeight={500}
              fontSize={"xx-small"}
              color={"secondary.300"}
            >
              {info.row.original.teamCode}
            </Text>
          </Stack>
        ),
        header: () => <span>Team</span>,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (values.id != null && values.id.length > 0) {
        // Update Action
        await EditTeamRoleServ({
          id: values.id,
          teamRoleName: values.teamRoleName,
        });
      } else {
        // Insert Action
        await AddTeamRoleServ({
          teamId: values.teamId,
          teamRoleCode: values.teamRoleCode,
          teamRoleName: values.teamRoleName,
        });
      }
    },
  ...
  const handleAddNew = () => {
    if (DataAuth && DataAuth.teamMember) {
      ...
      formik.setFieldValue("teamId", DataAuth.teamMember.id);
      formik.setFieldValue("teamRoleCode", "");
      formik.setFieldValue("teamRoleName", "");
      ModalForm.onOpen();
    } else {
      showToast({
        description: "Team ID is invalid",
  ...
  const [questionMsgDialog, setQuestionMsgDialog] = useState<string>("");
  const [captionDialog, setCaptionDialog] = useState<string>("");
  const [detailData, setdetailData] = useState<TeamRoleFullResponse | null>(
    null
  );

  const GetDetailTeamRole = async (
    id: string
  ): Promise<TeamRoleFullResponse | null> => {
    const PayloadList: PaggingListPayload = {
      search: "",
      limit: 1,
      page: 0,
  ...
  const AddTeamRoleServ = async (data: TeamRoleInsertPayload) => {
    const requestData = await InsertTeamRole(data, tokenData);
    const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
  ...
  const EditTeamRoleServ = async (data: TeamRoleUpdatePayload) => {
    const requestData = await UpdateTeamRole(data, tokenData);
    const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
  ...
  const DeleteTeamRoleServ = async (id: string) => {
    const requestData = await DeleteTeamRole(id, tokenData);
    const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
  ...
  const handleEditData = async (id: string) => {
    ...
    await delay(DELAY_MEDIUM);
    if (DataAuth && DataAuth.teamMember) {
      const GetData: TeamRoleFullResponse | null = await GetDetailTeamRole(id);
      if (GetData == null) return;
      formik.setFieldValue("id", GetData.id);
      formik.setFieldValue("teamId", DataAuth.teamMember.id);
      formik.setFieldValue("teamRoleCode", GetData.teamRoleCode);
      formik.setFieldValue("teamRoleName", GetData.teamRoleName);
      ModalForm.onOpen();
      setActionLoading(false);
    } else {
      showToast({
  ...
  const handleConfirmDeleteData = (data: TeamRoleFullResponse) => {
    setCaptionDialog("Confirm Delete");
    setQuestionMsgDialog(
      `Are you sure want to delete "${data.teamRoleName}"?\nWarning, it will affected User Team`
    );
    setOpenConfirmDeleteDialog(true);
    setdetailData(data);
  };
  ...
  const handleDeleteData = async () => {
    ...
    if (DataAuth && DataAuth.teamMember && detailData) {
      await DeleteTeamRoleServ(detailData.id);
    } else {
      showToast({
        description: "ID is invalid",
        statusToast: "error",
  ...
  return (
    <Flex as={Stack} w={"full"} spacing={2} pb={5}>
      <Text fontWeight={600}>Role Team Setting</Text>
      <Flex as={Wrap} justifyContent={"end"} px={0} w={"full"}>
        <Button
          size={"sm"}
          leftIcon={<FiRefreshCcw />}
          ...
          >
            ...
            <ModalBody w={"full"}>
              <Flex as={Stack} w={"full"} pt={4}>
                <FormControl
                  id="teamRoleCode"
                  isInvalid={formik.errors.teamRoleCode ? true : false}
                  isRequired
                >
                  <InputLayoutFull>
                    <FormLabel h={"full"} mt={2}>
                    ...
                    <Stack spacing={0}>
                      <Input
                        ...
                        onChange={(e) => {
                          const uppercaseValue = e.target.value.toUpperCase(); // Convert to uppercase
                          formik.setFieldValue("teamRoleCode", uppercaseValue); // Update Formik's value
                        }}
                        value={formik.values.teamRoleCode ?? ""}
                        placeholder="Team Role Code"
                        variant={
                ...
                <FormControl
                  id="teamRoleName"
                  isInvalid={formik.errors.teamRoleName ? true : false}
                  isRequired
                >
                  <InputLayoutFull>
                    <FormLabel h={"full"} mt={2}>
                    ...
                    <Stack spacing={0}>
                      <Input
                        ...
                        onChange={(e) => {
                          const uppercaseValue = e.target.value.toUpperCase(); // Convert to uppercase
                          formik.setFieldValue("teamRoleName", uppercaseValue); // Update Formik's value
                        }}
                        value={formik.values.teamRoleName ?? ""}
