...
const HeaderDataContent: HeaderContentProps = {
  titleName: "Kanban Example ALT",
  breadCrumb: ["Home", "Kanban ALT"],
};
...
const ResponseDataTasks: TaskInterface[] = [
  {
    id: generateUUIDV1(),
    text: "Design homepage",
    taskStatus: TASK_BOARD_STATUS_CODE_TODO,
    index: 0,
  },
  {
    id: generateUUIDV1(),
    text: "Develop login API",
    taskStatus: TASK_BOARD_STATUS_CODE_TODO,
    index: 1,
  },
  {
  ...
  },
  {
    id: generateUUIDV1(),
    text: "Pemberontakan",
    taskStatus: TASK_BOARD_STATUS_CODE_INPROGRESS,
    index: 0,
  },
  {
    id: generateUUIDV1(),
    text: "Bangunkan Jendral Jam 3 Pagi",
    taskStatus: TASK_BOARD_STATUS_CODE_INPROGRESS,
    index: 1,
  },
  {
    id: generateUUIDV1(),
    text: "Kabur ke Kamboja",
    taskStatus: TASK_BOARD_STATUS_CODE_INPROGRESS,
    index: 2,
  },
  {
    id: generateUUIDV1(),
    text: "Kudeta Uchiha",
    taskStatus: TASK_BOARD_STATUS_CODE_REVIEW,
    index: 0,
  },
];
...
function KanbanAltPage() {
  ...
  const [todoTasks, setTodoTasks] = useState<TaskInterface[]>([]);
  const [inProgressTasks, setInProgressTasks] = useState<TaskInterface[]>([]);
  const [inReviewTask, setInReviewTasks] = useState<TaskInterface[]>([]);
  const [doneTask, setDoneTasks] = useState<TaskInterface[]>([]);
  ...
  useEffect(() => {
    setisLoading(true);
    delay(1000);
    setTaskData(ResponseDataTasks);
    setChangedTask(null);
    setTodoTasks(
      ResponseDataTasks.filter(
        (x) => x.taskStatus === TASK_BOARD_STATUS_CODE_TODO
      )
    );
    setInProgressTasks(
      ResponseDataTasks.filter(
        (x) => x.taskStatus === TASK_BOARD_STATUS_CODE_INPROGRESS
      )
    );
    setInReviewTasks(
      ResponseDataTasks.filter(
        (x) => x.taskStatus === TASK_BOARD_STATUS_CODE_REVIEW
      )
    );
    setDoneTasks(
      ResponseDataTasks.filter(
        (x) => x.taskStatus === TASK_BOARD_STATUS_CODE_DONE
      )
    );
    setisLoading(false);
  }, [RefreshAction]);
  ...
  const handleDragStart = (task: TaskInterface, e: React.DragEvent) => {
    setDraggedTask(task);
  ...
  const handleDrop = (
    ...
    const setters = {
      ...
      inReview: setInReviewTasks,
      done: setDoneTasks,
    };

    const statusCodes = {
      toDo: TASK_BOARD_STATUS_CODE_TODO,
      inProgress: TASK_BOARD_STATUS_CODE_INPROGRESS,
      inReview: TASK_BOARD_STATUS_CODE_REVIEW,
      done: TASK_BOARD_STATUS_CODE_DONE,
    };

    const targetStatus = statusCodes[targetList];
    const targetTasks = taskLists[targetList];
    const { clientY } = e;

    let newIndex = -1;
    const taskBoundingRects = targetTasks.map((task) => {
      const taskElement = document.getElementById(`task-${task.id}`);
      return taskElement?.getBoundingClientRect() || null;
    });
    ...
    const currentTasks = taskLists[targetList];
    const currentHasTask = currentTasks.some(
      (task) => task.id === draggedTask.id
    );

    if (!currentHasTask) {
      const updatedTargetTasks = [...targetTasks];
      updatedTargetTasks.splice(newIndex, 0, updatedTask);
      setters[targetList](updatedTargetTasks);

      // Moved to new status
      const updatedDraggedTask = {
        ...draggedTask,
        taskStatus: mapTargetListToStatus(targetList), // helper function
        index: newIndex,
      };

      setters[targetList](updatedTargetTasks);

      // Remove dragged task from other lists
      Object.entries(taskLists).forEach(([key, list]) => {
        if (key !== targetList) {
          const updatedList = list.filter((task) => task.id !== draggedTask.id);
          setters[key as keyof typeof setters](updatedList);
        }
      });
      ...
      setChangedTask(updatedDraggedTask); // ✅ Store change
    } else {
      // Within same list, index might change
      if (draggedTask.index !== newIndex) {
        const updatedDraggedTask = { ...draggedTask, index: newIndex };
        updateTaskIndex(draggedTask.id, newIndex, setters[targetList]);
        setChangedTask(updatedDraggedTask); // ✅ Store change
      }
    }
...
const SubTasksInit: SubTaskTypes[] = [
  { id: generateUUIDV1(), text: "Sub Task 1", completed: false },
  { id: generateUUIDV1(), text: "Sub Task 2", completed: true },
  { id: generateUUIDV1(), text: "Sub Task 3", completed: false },
  { id: generateUUIDV1(), text: "Sub Task 4", completed: true },
];
