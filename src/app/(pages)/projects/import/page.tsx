...
export default function ProjectImportPage() {
  ...
  const processDataRows = (rawRows: Record<string, any>[], forAPI: boolean = false): Record<string, any>[] => {
    return rawRows.map(row => {
      const processedRow: Record<string, any> = {};
      Object.keys(row).forEach((key, index) => {
        const value = row[key];
        // Convert dates for date columns (AG, AH, AI, AJ are date columns)
        if (index >= 32 && index <= 35) { // AG=32, AH=33, AI=34, AJ=35
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportedData({
        requiredFlags,
        fieldKeys,
        dataRows: processDataRows((dataRows as Record<string, any>[]).filter(row => Object.values(row).some(val => val !== "")))
      });
  ...
  const handleImport = () => {
    ...
    const apiData = processDataRows(
      (importedData?.dataRows || []).filter(row => Object.values(row).some(val => val !== "")), 
      true // forAPI = true
    );
              ...
              >
                <CardBody>
                  ...
                  <VStack align="start" spacing={2}>
                    ...
                    <Text fontSize="sm" color={colorMode === "light" ? "blue.700" : "blue.200"}>
                      2. Fill in your project data following the template structure
                    </Text>
                    <Text fontSize="sm" color={colorMode === "light" ? "blue.700" : "blue.200"}>
              ...
              <CardBody>
                <VStack spacing={4} align="start">
                  ...
                  <VStack align="start" spacing={3}>
                    ...
                    <Box>
                      <Badge colorScheme="orange" mb={2}>Validation Rules</Badge>
                      <Text fontSize="sm" color="gray.600">
