...
export default function ProjectRegisterView({
  ...
  const { RegisterProjectNew, RegisterProjectDraft, UpdateProjectRegister, GetProjectCount, GetDetailById } = useProjects();
  const {
  ...
  useEffect(() => {
    ...
    // Load projectId from URL if exists
    if (typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search);
      const projectIdFromUrl = urlParams.get('id');
      if (projectIdFromUrl && !draftId && tokenData) {
        setDraftId(projectIdFromUrl);
        setIsDraft(true);
        loadProjectDraft(projectIdFromUrl);
      }
    }
  }, [DataAuth, tokenData, draftId]);
  // End SetUp auth data on current page

  const loadProjectDraft = async (projectId: string) => {
    const requestData = await GetDetailById(projectId, tokenData);
    
    if (requestData?.statusCode === RES_CODE_OK && requestData.data) {
      const projectData = requestData.data;
      
      // Load requirement if reqParentId exists
      if (projectData.reqParentId) {
        const reqData = await GetReqDetail(projectData.reqParentId, tokenData);
        if (reqData?.statusCode === RES_CODE_OK && reqData.data) {
          setDataRequirement(reqData.data);
        }
      }
      ...
      formik.setValues({
        ...
        projectRegisterDate: projectData.projectRegisterDate || "",
        projectClosedDate: projectData.projectClosedDate || null,
        projectAcquisitionCode: projectData.projectAcquisitionCode || null,
        projectCharasteristicCode: projectData.projectCharasteristicCode || null,
        projectSubCharasteristicCode: projectData.projectSubCharasteristicCode || null,
        proOwnerDirectorateId: projectData.proOwnerDirectorateId || null,
        proManageByDirectorateId: projectData.proManageByDirectorateId || null,
        proOwnerDivisionId: projectData.proOwnerDivisionId || "",
        proOwnerGroupId: projectData.proOwnerGroupId || "",
        proManageByDivisionId: projectData.proManageByDivisionId || "",
        proManageByGroupId: projectData.proManageByGroupId || "",
        proManageByTeamId: projectData.proManageByTeamId || "",
        userAssigns: projectData.userAssignment?.map((a) => ({ userId: a.userId })) || [],
        projectPlanWorkflowIds: projectData.projectWorkflowData?.map((w) => w.wfgId) || [],
        projectPlanWorkflowBacklogsIds: projectData.projectWorkflowProjectData?.map((w) => w.wfgId) || [],
        workProgramsBacklogs: [],
        workPrograms: [],
      });

      // Load team members
      if (projectData.userAssignment && projectData.userAssignment.length > 0) {
        const teamMembers = projectData.userAssignment.map((assign) => assign.userData);
        setChoosedMemberProjects(teamMembers.filter(Boolean));
      }

      // Load selected workflow IDs
      if (projectData.projectWorkflowData && projectData.projectWorkflowData.length > 0) {
        const workflowIds = new Set(projectData.projectWorkflowData.map((w) => w.wfgId));
        setSelectedWorkflowIds(workflowIds);
      }

      // Load backlogs if INTERNAL_DEVELOPMENT
      if (projectData.projectType === PROJECT_TYPE_INTERNAL_DEVELOPMENT && projectData.reqParentId) {
        // Load backlogs from requirement
        const backlogsData = await ListBacklog(
          {
            search: "",
            limit: 1000,
            page: 0,
            filterWhere: [
              { field: "reqId", operator: "=", value: projectData.reqParentId }
            ],
        ...
        if (backlogsData?.statusCode === RES_CODE_OK && backlogsData.data) {
          setDataBacklogsRequirement(backlogsData.data);
          
          // Set selected backlogs (those assigned to this project)
          const assignedBacklogIds = backlogsData.data
            .filter((b: any) => b.projectId === projectId)
            .map((b: any) => b.id);
          setSelectedBacklogIds(assignedBacklogIds);
        }
      }
  ...
  const RegisterProjectData = async (data: ProjectInsertPayload) => {
    ...
    const selectedBacklogsList = projectTypeRegister == PROJECT_TYPE_INTERNAL_DEVELOPMENT
      ? DataBacklogsRequirement.filter((b) => selectedBacklogIds.includes(b.id))
      : [];

    const backlogsProject: BacklogUpdatePayload[] = mapBacklogArrayToUpdatePayload(selectedBacklogsList);
  ...
  const handleSaveDraft = async (data: ProjectInsertPayload) => {
    const selectedBacklogsList = projectTypeRegister == PROJECT_TYPE_INTERNAL_DEVELOPMENT
      ? DataBacklogsRequirement.filter((b) => selectedBacklogIds.includes(b.id))
      : [];
    ...
    console.log("Organization Fields:", {
      proOwnerDirectorateId: payload.proOwnerDirectorateId,
      proOwnerDivisionId: payload.proOwnerDivisionId,
      proOwnerGroupId: payload.proOwnerGroupId,
      proManageByDirectorateId: payload.proManageByDirectorateId,
      proManageByDivisionId: payload.proManageByDivisionId,
      proManageByGroupId: payload.proManageByGroupId,
    });

    if (draftId) {
      const updatePayload: ProjectRegisterUpdatePayload = {
        id: draftId,
        ...payload,
        isSubmit: false
      };
      const requestData = await UpdateProjectRegister(updatePayload, tokenData);
      const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
      ...
      const requestData = await RegisterProjectDraft(payload, tokenData);
      const isErrorResponse = requestData?.statusCode !== RES_CODE_OK;
      ...
      if (isErrorResponse || !requestData) {
        ...
        const newProjectId = requestData.data as string;
        setDraftId(newProjectId);
        setIsDraft(true);

        // Update URL with projectId
        const currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set('id', newProjectId);
  ...
  const handleSubmitDraft = async (data: ProjectInsertPayload) => {
    if (draftId && isDraft) {
      const selectedBacklogsList = projectTypeRegister == PROJECT_TYPE_INTERNAL_DEVELOPMENT
        ? DataBacklogsRequirement.filter((b) => selectedBacklogIds.includes(b.id))
        : [];
      ...
      await RegisterProjectData(data);
    }
  };
  ...
  const [availableBacklogsFilter, setAvailableBacklogsFilter] = useState<string>("");
  // Pagination state for selected backlogs
  const [selectedBacklogsPagination, setSelectedBacklogsPagination] = useState<PaginationState>({
    pageIndex: 0,
    pageSize: 10,
  });
  ...
  const availableBacklogs = useMemo(
    () => {
      ...
      if (availableBacklogsFilter) {
        filtered = filtered.filter((backlog) =>
          backlog.backlogName.toLowerCase().includes(availableBacklogsFilter.toLowerCase()) ||
          backlog.backlogDesc?.toLowerCase().includes(availableBacklogsFilter.toLowerCase())
        );
      }
  ...
  const selectedBacklogsTable = useReactTable({
    ...
    columns: columnsData,
    pageCount: Math.ceil(selectedBacklogs.length / selectedBacklogsPagination.pageSize),
    state: {
  ...
  const handleSaveData = async () => {
    ...
    await handleSubmitDraft(formik.values);
            ...
            >
              <Button
                ...
                onClick={async () => {
                  ...
                  await handleSaveDraft(formik.values);
                }}
                isLoading={ActionLoading}
                px={8}
                size={"lg"}
              ...
              <Button
                ...
                size={"lg"}
              >
                {isDraft ? "Submit Draft" : "Submit Data"}
              </Button>
            </Flex>
          </GridItem>
                                ...
                                <Card rounded={radiusStyle}>
                                  <CardHeader>
                                    <HStack justify="space-between">
                                      <Box>
                                        <Heading size="md">Available Backlogs</Heading>
                                        <Text fontSize="sm" color="gray.500">
                                          Select backlogs to include in this project
                                        </Text>
                                      </Box>
                                      <HStack spacing={3}>
                                        <Input
                                          ...
                                          size="sm"
                                          onChange={(e) => setAvailableBacklogsFilter(e.target.value)}
                                        ...
                                        <Checkbox
                                          isChecked={
                                            ...
                                            selectedBacklogIds.length === availableBacklogs.length
                                          }
                                          isIndeterminate={
                                            selectedBacklogIds.length > 0 &&
                                            selectedBacklogIds.length < availableBacklogs.length
                                          }
                                          onChange={(e) => toggleAllAvailableBacklogs(e.target.checked)}
                                        >
                                          Select All
                                        </Checkbox>
                                      </HStack>
                                ...
                                {selectedBacklogs.length > 0 && (
                                  <Card rounded={radiusStyle} borderColor="blue.500" borderWidth="2px">
                                    <CardHeader>
                                      ...
                                      <Text fontSize="sm" color="gray.500">
                                        Edit details for selected backlogs that will be included in this project
                                      </Text>
                                    </CardHeader>
                                    <CardBody>
