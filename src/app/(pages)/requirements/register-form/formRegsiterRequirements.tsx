...
function RegisterRequirementFormPage({
  ...
  const nomorMemoRef = useRef<HTMLInputElement>(null);
  const nomorMemoCursorPosRef = useRef<number | null>(null);
  const {
    InsertReq,
    RegisterDraft,
    RegisterUpdate,
  ...
  const [approvalCountdown, setApprovalCountdown] = useState(5);
  ...
  // Countdown timer for approval request confirmation
  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (isApprovalRequestOpen) {
  ...
  const formik = useFormik<RequirementsInsertPayload>({
    ...
    onSubmit: async (values) => {
      ...
      if (!values.appLiveTargetDate) {
        showToast({
          description:
            "Target Live is required. Please complete Step 5 (Ringkasan Ruanglingkup).",
          statusToast: "warning",
        });
        return;
      }
    ...
    <LayoutAdmin>
                      ...
                      <Flex as={Stack} w={"full"} spacing={5}>
                        ...
                        >
                          <FormControl>
                            <InputLayoutFull>
                              <FormLabel h={"full"} mt={2}>
                                Grup Yang Melakukan <i>Approval</i>{" "}
                                <span style={{ color: "primary" }}>*</span>
                                <Tooltip
                                  label="Data Pengajuan Persetujuan terhadap Pemimpin Grup Terkait"
                                  fontSize="sm"
                                >
                                  <InfoOutlineIcon
                                    boxSize={3}
      ...
      >
        ...
        <ModalContent rounded={radiusStyle}>
          ...
          <ModalBody py={6}>
            <VStack spacing={4} align="stretch">
              <HStack spacing={2} align="flex-start">
                ...
                <Box>
                  <Text fontWeight="bold" color="orange.500">WARNING: Approval Request Action</Text>
                  <Text mt={2}>Are you sure you want to send this requirement for approval?</Text>
                </Box>
              </HStack>
                  ...
                  <HStack spacing={2} align="flex-start">
                    ...
                    <Box>
                      ...
                      <Text fontSize="sm" mt={1}>
                        Once submitted, the <strong>Scope of Work</strong> cannot be edited once it has been approved.
                      </Text>
                    </Box>
                  </HStack>
                </CardBody>
              ...
              <HStack spacing={2} align="flex-start">
                ...
                <Text fontSize="sm">
                  This action will initiate the formal approval process for your requirement submission.
                </Text>
              </HStack>
            </VStack>
          </ModalBody>
          <ModalFooter>
            <HStack spacing={3}>
              <Button leftIcon={<FiX />} onClick={onApprovalRequestClose}>
                Cancel
              </Button>
              <Button
                leftIcon={<FiCheck />}
                ...
                isDisabled={approvalCountdown > 0}
              >
                {approvalCountdown > 0 ? `Wait ${approvalCountdown}s` : `Send Request`}
              </Button>
            </HStack>
          </ModalFooter>
        </ModalContent>
...
const Section4RFCView = ({
  ...
  useEffect(() => {
    ...
    if (BacklogApps.length > 0 && BacklogChanges.length === 0 && isEditMode) {
      ...
      const backlogChangesData: BacklogChangesData[] = BacklogApps.map((b) => {
        ...
        const latestHistory =
          b.backlogHistories && b.backlogHistories.length > 0
  ...
  useEffect(() => {
    ...
    const updatedBacklogData: ReqBacklogPayload[] = BacklogChanges.map((dt) => {
